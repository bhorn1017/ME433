...
void drawChar(unsigned short x,unsigned short y, unsigned short color, unsigned char letter){
    letter = letter - 0x20; //shift over to make it match ASCII table
    
    //create loops indices
    int ii;
    int jj;
    
    //stores values of current column
    char value;
    
    for (ii=0;ii<5;ii++){
        value = ASCII[letter][ii];
        for (jj=0;jj<8;jj++){
            if (((value >> jj) & 0x01) ==1){ //check if the pixel should be drawn to
                LCD_drawPixel(x+ii,y+jj,color);
            } //end of if statement
            else {  //fill in the background color where the pixels aren't drawn
...
void drawString(unsigned short x,unsigned short y, unsigned short color,char *m){
    ...
    int i; //create loop index
    char letter; //stores the current letter from the array
    for (i=0;i<20;i++){
        letter = m[i];
        if (letter == '\0'){ //end condition
            break;
        }
        else {
